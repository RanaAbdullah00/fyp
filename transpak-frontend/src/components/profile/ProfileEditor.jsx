...
const ProfileEditor = ({ showTabs, onSaved }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (status === 503) {
        ...
        notifyError(raw || t('profile.saveFailed'));
      }
    } finally {
      setLoading(false);
    }
    ...
    profileComplete && showTabs ? (
      <div className="small text-body">
        ...
        {!cnicFrontUrl && !cnicBackUrl ? (
          <p className="text-muted small mb-0 mt-2">{t('profile.noCnicScans')}</p>
        ) : (
  ...
  const rolePane = (
    <div className="small text-body">
      ...
      <p className="mb-1">
        <strong>{t('profile.activeRole')}:</strong> {user?.activeRole || t('common.emDash')}
      </p>
      <p className="mb-0">
        <strong>{t('profile.rolesLabel')}:</strong> {(user?.roles || []).join(', ') || t('common.emDash')}
      </p>
    </div>
  );
  ...
  const statusPane =
    profileComplete && showTabs ? (
      <div className="small text-body">
        <div className="mb-3">
          <span className="badge bg-success rounded-pill px-3 py-2">{t('profile.profileCompleteBadge')}</span>
        </div>
        <p className="mb-1">
          <strong>{t('auth.fullName')}:</strong> {form.full_name || t('common.emDash')}
        </p>
      ...
      <div className="small text-body">
        <p className="text-muted mb-2">{t('profile.completionHint')}</p>
        {missingProfilePieces.length === 0 ? (
          <p className="mb-3">{t('profile.completionServerHint')}</p>
        ) : (
          <ul className="list-unstyled mb-3">
            {missingProfilePieces.map((item) => (
              <li key={item.key} className="mb-1 d-flex align-items-center gap-2">
  ...
  if (showTabs) {
    return (
      <div className="tp-profile-editor-tabs d-flex flex-column h-100">
        <ul className="nav nav-tabs flex-shrink-0 px-2 pt-2 tp-profile-editor-tabs__nav">
          {tabBtn('basic', t('profile.basicInfo'))}
          {tabBtn('cnic', t('auth.cnic'))}
          {tabBtn('role', t('profile.roleInfo'))}
          {tabBtn('status', t('profile.completion'))}
        </ul>
        <div className="tp-profile-editor-tabs__pane flex-grow-1 px-3 py-2 overflow-hidden d-flex flex-column">
          <div className="tp-profile-sheet-pane-inner w-100 flex-grow-1 d-flex flex-column justify-content-start">
            {tab === 'basic' && profileFields}
            {tab === 'cnic' && cnicPane}
            {tab === 'role' && rolePane}
